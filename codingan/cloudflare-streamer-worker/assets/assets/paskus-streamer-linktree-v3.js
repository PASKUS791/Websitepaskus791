(function () {
  ...
  function profileMedia(profile) {
    ...
    return { ...localMedia, ...(profile.media || {}) };
  }
  ...
  function applyProfile(profile) {
    ...
    document.documentElement.style.setProperty(
      "--page-bg-image",
      cssImage(media.background || "/assets/about-us-bg-v1.webp"),
    );
    document.documentElement.style.setProperty(
      "--profile-bg-image",
      cssImage(media.background || "/assets/paskus-landing-loop-poster-v2.jpg"),
    );
    ...
    const webm = media.highlightWebm || highlight.webm || highlight.video;
    const mp4 = media.highlightMp4 || highlight.mp4 || highlight.fallback;

    if (video) {
      const directVideo = video.canPlayType("video/webm; codecs=\"vp9, opus\"") && webm ? webm : mp4 || webm;
