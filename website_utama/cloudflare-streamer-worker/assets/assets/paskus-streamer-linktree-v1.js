(function () {
  ...
  function applyProfile(profile) {
    ...
    setAttr(".avatar-ring img", "src", profile.avatar || "/recruitment-webhook-logo.png");
    ...
    if (video && highlight.poster) {
      video.poster = highlight.poster;
    }
    if (sources[0] && highlight.video) {
      sources[0].src = highlight.video;
    }
    if (sources[1] && highlight.fallback) {
      sources[1].src = highlight.fallback;
    }
    if (video && (highlight.video || highlight.fallback)) {
      video.load();
    }
